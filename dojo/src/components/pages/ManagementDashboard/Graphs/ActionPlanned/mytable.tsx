...
const MyTable = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get("http://172.25.0.51:8000/api/actions/");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`http://172.25.0.51:8000/api/actions/${editingId}/`, formData);
      } else {
        await axios.post("http://172.25.0.51:8000/api/actions/", formData);
      }
      fetchData();
      closeModal();
    } catch (error) {
  ...
  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to delete this?")) {
      try {
        await axios.delete(`http://172.25.0.51:8000/api/actions/${editingId}/`);
        fetchData();
        closeModal();
      } catch (error) {
