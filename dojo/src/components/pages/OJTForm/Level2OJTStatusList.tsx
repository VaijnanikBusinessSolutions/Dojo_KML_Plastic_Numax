...
const Level2OJTStatusList: React.FC = () => {
  ...
  // separate loading state so we don't hide the UI while searching
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Search Inputs
  const [searchTerm, setSearchTerm] = useState(""); 
  const [debouncedSearch, setDebouncedSearch] = useState(""); // This is what the API actually listens to
  ...
  /* 
     1. DEBOUNCE TIMER 
     This waits 500ms after you stop typing to update 'debouncedSearch'.
     This prevents the API from firing on every single letter.
  */
  useEffect(() => {
  ...
  /* 
     2. FETCH DATA 
     This listens to 'debouncedSearch' (not searchTerm), so it doesn't run instantly.
  */
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Use the DEBOUNCED value for the API
    ...
    fetchData();
  }, [debouncedSearch, statusFilter, departmentFilter]); // Dependency matches the debounce
  ...
  const filtered = useMemo(() => {
    return data.filter(item => {
      // Immediate client-side filtering (optional, makes it feel snappier)
      const matchesSearch =
        !searchTerm ||
        (item.emp_id ?? "").toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.trainee_name.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const exportToExcel = async () => {
    ...
    const headerRow = worksheet.getRow(5);
    const headers = ["Emp ID", "Trainee Name", "Trainer", "Department", "Line", "Station", "Status", "Result"];
    headers.forEach((h, i) => {
      const cell = headerRow.getCell(i + 1);
      cell.value = h;
      cell.font = { bold: true, color: { argb: "FFFFFFFF" } };
    ...
    // Add data logic here (simplified for brevity, copy your previous export logic if needed)
    filtered.forEach((item) => {
      worksheet.addRow([
        item.emp_id, item.trainee_name, item.trainer_name, item.category, item.line, item.station_name, item.calculated_status, item.calculated_result
      ]);
    });

    const buffer = await workbook.xlsx.writeBuffer();
    const blob = new Blob([buffer], { type: "application/octet-stream" });
    saveAs(blob, `OJT_Level2_Report.xlsx`);
  };
  ...
  const renderResult = (result: "Pass" | "Fail" | "N/A") => {
    if (result === "Pass") return <div className="px-3 py-1 bg-emerald-100 text-emerald-800 rounded-full text-xs font-bold">Pass</div>;
    if (result === "Fail") return <div className="px-3 py-1 bg-rose-100 text-rose-800 rounded-full text-xs font-bold">Fail</div>;
    return <span className="text-gray-400 text-xs">N/A</span>;
  };

  const getDepartmentBadgeStyle = (dept: string) => {
    const hash = dept.split('').reduce((a, b) => a + b.charCodeAt(0), 0);
    const colors = ["bg-blue-100 text-blue-800", "bg-purple-100 text-purple-800", "bg-pink-100 text-pink-800", "bg-indigo-100 text-indigo-800"];
    return colors[hash % colors.length];
  };
  ...
  // NOTE: I removed the "if (isLoading) return ..." block. 
  // This ensures the page never wipes out while you type.

  return (
    <div className="w-full bg-gradient-to-br from-gray-50 via-white to-gray-50 py-8 px-4 sm:px-6 lg:px-8 min-h-screen">
      <div className="w-full mx-auto space-y-6">

        {/* HEADER */}
        <div className="relative bg-gradient-to-r from-indigo-600 to-blue-600 rounded-2xl shadow-xl p-8 text-white">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            <div className="flex items-center gap-4">
              <Award className="w-10 h-10 text-white/90" />
              <div>
                <h1 className="text-3xl font-bold">OJT Level 2 Dashboard</h1>
                <p className="text-indigo-100 text-sm mt-1">Real-time status tracking</p>
              </div>
            </div>
            {selectedView === "OJT" && (
              <button onClick={exportToExcel} className="flex items-center gap-2 px-4 py-2 bg-white/20 hover:bg-white/30 rounded-lg transition backdrop-blur-md">
        ...
        {selectedView === "OJT" ? (
          <>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <div className="flex flex-col md:flex-row gap-4">
                {/* SEARCH INPUT - Controlled and Debounced */}
                <div className="flex-1 relative">
                  ...
                  />
                  {/* Small spinner inside input if loading */}
                  {isLoading && (
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden relative min-h-[400px]">
              ...
              {/* Overlay Loader: Shows ON TOP of table, doesn't delete it */}
              {isLoading && (
                <div className="absolute inset-0 bg-white/50 backdrop-blur-[1px] z-20 flex items-center justify-center">
                  <div className="flex flex-col items-center">
                     ...
                     <span className="text-xs font-bold text-indigo-700">Updating...</span>
                  </div>
                </div>
              )}
                    ...
                    {filtered.length === 0 && !isLoading ? (
                      ...
                      filtered.map((item) => (
                        <tr key={item.id} className="hover:bg-gray-50">
                          <td className="px-6 py-4 font-mono text-gray-600">{item.emp_id || "-"}</td>
                          <td className="px-6 py-4 font-medium text-gray-900">{item.trainee_name}</td>
                          <td className="px-6 py-4 text-gray-600">{item.trainer_name}</td>
                          <td className="px-6 py-4"><span className={`px-2 py-1 rounded text-xs ${getDepartmentBadgeStyle(item.category)}`}>{item.category}</span></td>
          ...
          <div className="bg-white p-10 text-center rounded-xl shadow-sm">
            <p className="text-gray-500">TenCycle View Under Construction</p>
          </div>
        )}
      </div>
    </div>
